...
\section{Create a clear structure in LaTeX}
...
\begin{itemize}
    ...
    \item \texttt{toc.tex} - This file refers to individual files so I have a single file where to find my table of contents easily.
    \item \texttt{meta.tex} - I store meta information, i.e., title, author etc. to this file so they are easy to find.
    \item \texttt{chapters/00-abstract.tex} - If your work has an extended abstract, as is typical for theses, it may make sense to separate it into a file.
...
Note that if I am working on a paper, I tend to use the name \texttt{sections} instead of \texttt{chapters} since that maps better to the nomenclature used there.
Chapters are more natural choice for books like the one I am writing here.

I use a declaration like this for each chapter at \texttt{toc.tex} to import them:
...
The same idea works for importing the toc file and meta information to my \texttt{main.tex} and you will have find the right spots in your initial template for this purpose.
...
Another option to \texttt{hyperref} is known as \href{https://www.dr-qubit.org/cleveref.html}{cleveref}\footnote{\url{https://www.dr-qubit.org/cleveref.html}} and in some ways it goes beyond \texttt{hyperref} as it supports more complex ways of referring to your content.
I have found \texttt{hyperref} to be enough for simple use cases but when you want to do something more advanced then it may be worth it to refactor your code to use \texttt{cleveref} or use it out of the box.
...
% Another good thing to keep in mind is that since occasionally a paper to cite might have even tens of authors, you can condense them with ", and others" kind of syntax at the author field.

\section{Cite in LaTeX}
...
\begin{description}
    ...
    \item[\textbf{\textbackslash citet}] is a textual citation and it works well when your citation is the subject of a sentence. Note that this command does not always work as it depends on the citation style of your project.
\end{description}
...
You can see visual examples of citations in chapter \nameref{ch:citing}.

\section{Keep track of things to do with todonotes}

I recommend using a LaTeX package, such as \href{https://ctan.org/pkg/todonotes}{todonotes}\footnote{\url{https://ctan.org/pkg/todonotes}} to track things to do within a document and also to explain what you still have to cover.
Due to these factors, using \texttt{todonotes} can be particularly useful when collaborating with other people.
I prefer to set \texttt{\textbackslash setuptodonotes\{inline\}} as setting todos inline keeps the todos within text itself at the cost of breaking your layout slightly as this takes more vertical space.

\section{Comment out swaths of lines easily}

Although LaTeX allows you to comment out individual lines of code using the \texttt{\%} character, it gets wieldy do use when you want to comment out larger entities at once.
For this reason I prefer to use \href{https://ctan.org/pkg/comment}{comment}\footnote{\url{https://ctan.org/pkg/comment}} package as it gives block-level (\texttt{\textbackslash \{begin\}\{comment\}} and \texttt{\textbackslash \{end\}\{comment\}}) support for comments.
...
Another approach is to generate LaTeX syntax from your tooling and then put the resulting code to separate LaTeX files that you then use \texttt{\textbackslash input} where needed.
...
\section{Highlight code examples}
...
\section{Keep publication guidelines in mind}

Especially technical publications often offer LaTeX, and even Overleaf, templates that you should use to author your papers.
It is likely that you will have to modify these templates to your liking structurally at least.
Occasionally these templates come with specific technical limitations that are covered within them.
For example, you may have to use a specific LaTeX compiler for the templates to work or you have to avoid using specific packages altogether.
It is particularly important to pay attention to LaTeX errors and warnings before you submit since often there is something to fix and especially overfull warnings can point out to small layout issues you can fix by altering wording or by pointing out where to break a word assuming there are some LaTeX does not know how to hyphenate.
By following template rules, you make it easier for the publisher and avoid additional work related to tweaking the output.
